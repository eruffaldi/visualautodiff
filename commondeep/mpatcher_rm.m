...
%w = double(as)*Sel.A';
if isa(as,'gpuArray')
    w = gathermatrix_rm(Sel.pickidx,gather(as),length(Sel.pickidx));
else
    if isstruct(Sel)
        % ALWAYS MATLAB path
        f = Sel.gather;
        ...
        if coder.target('MATLAB')
            ...
            w = gathermatrix_rm(Sel,as,length(Sel));
...
%TODO            void gathermatrix_float(const float * pdata,int rows,int cols,const int32_t * psubs,int nsubs,float * pout,int outcols);
            ...
            coder.ceval(['gathermatrix_rm_' class(w)],coder.rref(as),rows,cols,coder.rref(Sel),nsubs,coder.wref(as),outcols); % c version
        else
            % slow fallback
            w = gathermatrixmat_rm(Sel,as,length(Sel));            
        end
    end
end
