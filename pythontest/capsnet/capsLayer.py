...
class CapsConv(object):
    ...
    Returns:
        A 4-D tensor.
    '''
    def __init__(self, num_units, BS, with_routing=True):
    ...
    def __call__(self, input, num_outputs, kernel_size=None, stride=None):
        ...
        BS = self.BS
        q = 1152
        print ("CapsConv",input.get_shape())


        if not self.with_routing:
            # the PrimaryCaps layer
            # input: [batch_size, 20, 20, 256]
            assert input.get_shape() == [BS, 20, 20, 256]
            ...
            for i in range(self.num_units):
                ...
                with tf.variable_scope('ConvUnit_' + str(i)):
                    caps_i = tf.contrib.layers.conv2d(input,
                                                      ...
                                                      self.stride,
                                                      padding="VALID")
                    caps_i = tf.reshape(caps_i, shape=(BS, -1, 1, 1))
                    capsules.append(caps_i)
            print ("capules",capsules[0].get_shape())
            #assert capsules[0].get_shape() == [BS, q, 1, 1]

            # [batch_size, 1152, 8, 1]
            capsules = tf.concat(capsules, axis=2)
            capsules = squash(capsules)
            #assert capsules.get_shape() == [BS, q, 8, 1]

        else:
            # the DigitCaps layer
            # Reshape the input into shape [batch_size, 1152, 8, 1]
            self.input = tf.reshape(input, shape=(BS, -1, 8, 1))

            # b_IJ: [1, num_caps_l, num_caps_l_plus_1, 1]
            b_IJ = tf.zeros(shape=[1, q, 10, 1], dtype=np.float32)
            capsules = []
            for j in range(self.num_outputs):
                with tf.variable_scope('caps_' + str(j)):
                    caps_j, b_IJ = capsule(BS, input, b_IJ, j)
                    capsules.append(caps_j)

            # Return a tensor with shape [batch_size, 10, 16, 1]
            capsules = tf.concat(capsules, axis=1)
            #assert capsules.get_shape() == [BS, 10, 16, 1]

        return(capsules)


def capsule(BS, input, b_IJ, idx_j):
    ...
    with tf.variable_scope('routing'):
        ...
        for r_iter in range(cfg.iter_routing):
            ...
            print ("c_IJ shape",c_IJ.get_shape())
            #assert c_IJ.get_shape() == [1, q, 10, 1]
            ...
            #assert c_Ij.get_shape() == [1, q, 1, 1]

            s_j = tf.multiply(c_Ij, u_hat)
            s_j = tf.reduce_sum(tf.multiply(c_Ij, u_hat),
                                axis=1, keep_dims=True)
            assert s_j.get_shape() == [BS, 1, 16, 1]
            ...
            v_j_tiled = tf.tile(v_j, [1, q, 1, 1])
            u_produce_v = tf.matmul(u_hat, v_j_tiled, transpose_a=True)
            assert u_produce_v.get_shape() == [BS, 1152, 1, 1]
