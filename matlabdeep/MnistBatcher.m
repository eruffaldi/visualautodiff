...
classdef MnistBatcher < handle
    ...
    methods
        ...
        function [i,l,lh] = next(obj,items)
            left = min(items,n-obj.iterations);
            if left == 0
                obj.iterations = 0;
                obj.epoch = obj.epoch + 1;
                left = min(items,obj.n);
            end
            i = obj.allimages(obj.iterations:obj.iterations+left-1,:);
            l = obj.alllabels(obj.iterations:obj.iterations+left-1,:);
            lh = obj.alllabelshot(obj.iterations:obj.iterations+left-1,:);
            obj.iterations = obj.iterations + left;
        end   
